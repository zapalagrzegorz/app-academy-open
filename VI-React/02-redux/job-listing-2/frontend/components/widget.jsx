...
class Widget extends React.Component {
  constructor(props) {
    ...
    // Registers callbacks to be triggered whenever the store updates. Returns a function, which when invoked, unsubscribes the callback function from the store.
  ...
  async fetchJobListings(city) {
    const response = await fetch(
      `https://79vzv34gc4.execute-api.us-west-1.amazonaws.com/default/jobListings?location=${city}`
    );
    const jsonResp = await response.json();
    this.props.store.dispatch(this.selectLocation(city, jsonResp));
  }
