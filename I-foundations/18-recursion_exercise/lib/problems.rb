...
def reverse_string(str)
  ...
  return str[str.length - 1] << reverse_string(str[0..str.length - 2])
end
...
def flatten(data)
  ...
  data.each do |el|
    # concat VS << => concating is NOT appending
    #  [1] += [2] => [1,2] -> += will loop for you
    #  [1] << [2] => [1, [2]] -> appending object
    outputArr += flatten(el)
  end

  outputArr
  # return [data, flatten(data[1...data.length])]
end
